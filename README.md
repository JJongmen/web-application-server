...
* 공부할 내용
  * 현재 서버 저장소는 서버를 재시작할 경우 세션의 아이디값들이 전부 초기화되버리는데 브라우저의 세션 아이디값은 남아있는 상황이다.
  * 서버 종료 시 브라우저의 세션아이디값을 지울 수 있는 것인지, 아니면 저장소가 초기화안되도록 만들어야하는지, 아니면 다른 해결방안이 있는지 모색해야겠다.